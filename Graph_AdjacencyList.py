from progetto.Graph import GraphBase, Edge, Node

from progetto.DoubleLinkedList import ListaDoppiamenteCollegata as List
...
class GraphAdjacencyList(GraphBase):
    ...
    def hasCycleDFSsubGraph(self, v, visited, parent):
        """
        Inizializzo il nodo da cui faccio partire la visita DFS su True, dopo di che ricorro su tutti i vertici
        adiacenti; se un vertice adiacente è visitato(=True) e non è genitore del vertice corrente, c'è un ciclo.
        Altrimenti ritorno False. In questa fase mi avvalgo del sottografo composto da v e i vertici da esso
        raggiungibili.
        """
        visited[v] = True
        for adjNode in self.getAdj(v):
          if visited[adjNode] == False:
            if self.hasCycleDFSsubGraph(adjNode, visited, v):
                return True
            elif parent != adjNode:
                return True
        return False
